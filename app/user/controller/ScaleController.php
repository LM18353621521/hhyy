...
class ScaleController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        if($this->inc_type=='frozen'){
            $list = $usersQuery->alias('a')
                ->join('user u', 'a.admin_id=u.id', 'left')
                ->join('user_visit v','a.visit_id=v.id','left')
                ->field('a.*,u.user_login as user_nickname,v.visit_time,v.visit_times')
                ->whereOr($keywordComplex)->where($where)->order("create_time DESC")->paginate(15)->each(function($item, $key){
                    $score_item = unserialize($item['score_item']);
                    $item['score1'] = $score_item[1];
                    $item['score2'] = $score_item[2];
                    return $item;
                });
        }else{
            $list = $usersQuery->alias('a')
    ...
    public function edit()
    {
        ...
        if ($data) {
            if ($inc_type == "mds_updrs") {
                ...
                $data['score_item'] = unserialize($data['score_item']);
                ...
                $data['items'] = unserialize($data['items']);
            } elseif ($inc_type == "frozen") {
                $data['items'] = unserialize($data['items']);
                foreach ($data['items'] as $key=>$val){
                    $data['items'][$key] = (int)$val;
                }
                $data['score_item'] = unserialize($data['score_item']);
            } elseif ($inc_type == "stroop") {
                $data['items'] = unserialize($data['items']);
            } elseif ($inc_type == "wais") {
                $data['items'] = unserialize($data['items']);
            }
        }
